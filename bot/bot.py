...
import data_service
import jsonmap
from extra import fb, twitch, features
...
async def latest_results(channel):
    ...
    for i in reversed(range(0, len(matches))):
        ...
        if not match_id in match_results_storage:
            ...
            if not initial:
                result_mark = helpers.get_result_marks(matches[i])
                await channel.send(
                    result_mark.mark + data_service.format_result(matches[i])
                )
                await channel.send(result_mark.gif)
...
async def handle_team_record(message):
    ...
    if len(command) < 2:
        ...
        team = " ".join(command[1:])
        await single_channel.send(
            "Getting records for " + team + "\n" + "Please wait..."
        )
...
async def handle_history(message):
    ...
    for batch in match_batches:
        ...
        for match in batch:
            match_results += (
                helpers.get_match_mark(match) + data_service.format_result(match) + "\n"
            )
